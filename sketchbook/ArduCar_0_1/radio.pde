...
void read_radio()
{
	...
	radio_in[CH_UNUSED] = timer4diff * .5;

	#if MIXING_MODE == 0
		radio_in[CH_STEERING] = ch1_temp;
		radio_in[CH_THROTTLE_R] = ch2_temp;
	#endif
	
	#if  MIXING_MODE == 1
		radio_in[CH_STEERING] = REVERSE_ELEVONS * (REVERSE_CH2_ELEVON*(ch2_temp-elevon2_trim) - REVERSE_CH1_ELEVON*(ch1_temp-elevon1_trim))/2 + 1500;
		radio_in[CH_THROTTLE_R] = (REVERSE_CH2_ELEVON*(ch2_temp-elevon2_trim) + REVERSE_CH1_ELEVON*(ch1_temp-elevon1_trim))/2 + 1500;
	#endif
...
void trim_control_surfaces()
{
	...
	// ---------------------------------
		...
		//We cannot use radio_in[CH_STEERING] and radio_in[CH_THROTTLE_R] values from read_radio() because the elevon trim values have changed
		radio_trim[CH_STEERING] = REVERSE_ELEVONS * (REVERSE_CH2_ELEVON*(ch2_temp-elevon2_trim) - REVERSE_CH1_ELEVON*(ch1_temp-elevon1_trim))/2 + 1500;
		radio_trim[CH_THROTTLE_R] = (REVERSE_CH2_ELEVON*(ch2_temp-elevon2_trim) + REVERSE_CH1_ELEVON*(ch1_temp-elevon1_trim))/2 + 1500;
	#endif
	#if MIXING_MODE == 0
		radio_trim[CH_STEERING] = radio_in[CH_STEERING];
		radio_trim[CH_THROTTLE_R] = radio_in[CH_THROTTLE_R];
	#endif
	
	radio_trim[CH_UNUSED] = radio_in[CH_UNUSED];
}
...
void trim_radio()
{
	...
	// ---------------------------
	while(radio_in[CH_STEERING] < 900 && radio_in[CH_THROTTLE_R] < 900){
		read_radio();
		delay(20);
	}
	...
	// -----------------------------
	radio_trim[CH_STEERING] 	= constrain(radio_trim[CH_STEERING], 950, 2050);
	radio_trim[CH_THROTTLE_R] 	= constrain(radio_trim[CH_THROTTLE_R], 950, 2050);
}
...
void read_radio_limits()
{
	...
	// -------------------------------------------------
	radio_min[CH_STEERING] = radio_in[CH_STEERING] - 150;
	radio_max[CH_STEERING] = radio_in[CH_STEERING] + 150;

	radio_min[CH_THROTTLE_R] = radio_in[CH_THROTTLE_R] - 150;
	radio_max[CH_THROTTLE_R] = radio_in[CH_THROTTLE_R] + 150;
	...
	while(counter < 50){
		...
		// --------------------
		if (radio_in[CH_STEERING] > 1000 && radio_in[CH_STEERING] < 2000){
			radio_min[CH_STEERING] = min(radio_in[CH_STEERING], radio_min[CH_STEERING]);
			radio_max[CH_STEERING] = max(radio_in[CH_STEERING], radio_max[CH_STEERING]);
		}
		
		if (radio_in[CH_THROTTLE_R] > 1000 && radio_in[CH_THROTTLE_R]< 2000){
			radio_min[CH_THROTTLE_R] = min(radio_in[CH_THROTTLE_R], radio_min[CH_THROTTLE_R]);
			radio_max[CH_THROTTLE_R] = max(radio_in[CH_THROTTLE_R], radio_max[CH_THROTTLE_R]);
		}
		if(radio_in[CH_THROTTLE_R] < (radio_min[CH_THROTTLE_R] + 30) || radio_in[CH_THROTTLE_R] > (radio_max[CH_THROTTLE_R] -30)){
			Serial.print(".");
			counter++;
		}else{
			if (counter > 0)
