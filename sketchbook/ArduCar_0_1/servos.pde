...
void set_servos_4()
{
		...
		if(imu_ok == false && control_mode > MANUAL){	        //  We have lost the IMU - Big trouble
			servo_out[CH_STEERING] = 0;   							//  If we have lost imu we will probably crash.  
			servo_out[CH_THROTTLE_R] = 0;							//  Neutralize controls, throttle off
	...
	#if MIXING_MODE == 0
		set_ch1_degrees(servo_out[CH_STEERING]); // 45 ° = right turn (unless reversed)
		set_ch2_degrees(servo_out[CH_THROTTLE_R]);
	#endif
	
	/*Elevon mode*/ // 
	#if MIXING_MODE == 1
		set_ch1_degrees(REVERSE_ELEVONS * (servo_out[CH_THROTTLE_R] - servo_out[CH_STEERING]));	
		set_ch2_degrees(servo_out[CH_THROTTLE_R] + servo_out[CH_STEERING]);
	#endif

	set_ch4_degrees(servo_out[CH_UNUSED]);
	update_throttle();
}
...
#if MIXING_MODE == 0
	radio_out[CH_STEERING] = radio_trim[CH_STEERING] + ((float)REVERSE_ROLL * deg * 11.111f);
#endif

#if MIXING_MODE == 1
	radio_out[CH_STEERING] =  elevon1_trim + ((float)REVERSE_CH1_ELEVON  * deg * 11.111f);    //
#endif
	radio_out[CH_STEERING] = constrain(radio_out[CH_STEERING], 	radio_min[CH_STEERING], 	radio_max[CH_STEERING]);
	radio_out[CH_STEERING] = constrain(radio_out[CH_STEERING], 	1000, 	2000);
	OCR1A = radio_out[CH_STEERING] * 2;	//OCR1A is the channel 1 pulse width in half microseconds
}
...
#if MIXING_MODE == 0
	radio_out[CH_THROTTLE_R] = radio_trim[CH_THROTTLE_R] + ((float)REVERSE_PITCH * deg * 11.111f);
#endif

#if MIXING_MODE == 1
	radio_out[CH_THROTTLE_R] =  elevon2_trim + ((float)REVERSE_CH2_ELEVON * deg * 11.111f);
#endif

	radio_out[CH_THROTTLE_R] = constrain(radio_out[CH_THROTTLE_R], 	radio_min[CH_THROTTLE_R], 	radio_max[CH_THROTTLE_R]);
	radio_out[CH_THROTTLE_R] = constrain(radio_out[CH_THROTTLE_R], 	1000, 	2000);
	OCR1B = radio_out[CH_THROTTLE_R] * 2;
}
...
void set_ch4_degrees(float deg){
	...
	radio_out[CH_UNUSED] = radio_trim[CH_UNUSED] + ((float)REVERSE_RUDDER * deg * 11.111f);
	//Serial.print("\tradio_out: ");
	//Serial.print(radio_out[CH_UNUSED],DEC);
	radio_out[CH_UNUSED] = constrain(radio_out[CH_UNUSED], 	radio_min[CH_UNUSED], 	radio_max[CH_UNUSED]);
	//Serial.print("\tradio_out: ");
	//Serial.print(radio_out[CH_UNUSED],DEC);
	//Serial.print(" : ");
	
	uint16_t timer_out 	= radio_out[CH_UNUSED] % 512; 
	timer_ovf_b 		= radio_out[CH_UNUSED] / 512;
	timer_out >>= 1;
