...
GPS Message format (update current waypoint)
0-3		 Header "DIYd"
4						Payload length = 14
5						Message ID = 5
6-9		WP longitude			Integer (value*10**7)
10-13		WP latitude			Integer (value*10**7)
14,15		WP altitude			Integer (meters*10)
16,17		WP Index			not used (In future, allow user to specify index into waypoint buffer)
18,19		gps course			not used
20,21		checksum
*/
...
void GPS_WP_join_data()
{
	int j=0;				
	new_WP.lng = join_4_bytes(&IMU_buffer[j]);		// Lat and Lon * 10**7
	j += 4;
	new_WP.lat = join_4_bytes(&IMU_buffer[j]);
	j += 4;

	intUnion.byte[0] = IMU_buffer[j++];
	intUnion.byte[1] = IMU_buffer[j++];
	new_WP.alt = (long)intUnion.word * 10;		 //	Altitude in meters * 100 
        init_home();                                      //     Change the home coordinate
